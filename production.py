...
class Migrator:
    ...
    def clean_and_prepare_data(self, df):
        ...
        text_fields = ['employer_state', 'case_status', 'employer_city']
        for field in text_fields:
            if field in df_filtered.columns:
                df_filtered[field] = df_filtered[field].astype(str).str.strip().str.upper()

        # Handle numeric fields
        numeric_fields = ['naics_code', 'prevailing_wage']
...
class SQLiteH1BReader:
    ...
    def get_cap_exempt_institutions(_self):
        ...
        FROM lca_data
        WHERE 
            CAST(naics_code AS TEXT) LIKE '611%'
            OR naics_code = 541711
            OR naics_code = 541712
            OR CAST(naics_code AS TEXT) LIKE '622%'
            OR naics_code = 927110
            OR CAST(naics_code AS TEXT) LIKE '9271%'
...
def ensure_database_exists():
    """Ensure database exists, create if needed for Streamlit Cloud deployment"""
    db_path = DB_FILE_PATH
    
    if Path(db_path).exists():
        return True, f"Database ready at {db_path}"
    
    # Database doesn't exist - try to create it
    st.info("🔄 Database not found. Checking for Excel file to create database...")
    ...
    if Path(EXCEL_FILE_PATH).exists():
        excel_files.append(Path(EXCEL_FILE_PATH))
    
    if not excel_files:
        return False, "No Excel file found for database creation"
    ...
    try:
        ...
        with st.spinner(f"🚀 Creating database from {excel_file.name}..."):
            success = migrator.migrate_excel_to_db(str(excel_file), SHEET_NAME)
            
        if success:
            return True, f"✅ Database created successfully from {excel_file.name}"
        else:
            return False, f"❌ Failed to create database from {excel_file.name}"
            
    except Exception as e:
        return False, f"Error creating database: {str(e)}"

def show_setup_instructions():
    """Show setup instructions when database cannot be created"""
    st.error("🚫 Database Setup Required")
    
    st.markdown("""
    ### 📋 Quick Setup Guide
    
    Your H-1B analysis dashboard needs data to run. Here's how to set it up:
    """)
    
    with st.expander("🔧 Setup Instructions", expanded=True):
        st.markdown("""
        **Option 1: For Streamlit Cloud Deployment**
        1. Include your Excel file in your GitHub repository
        2. Update the `EXCEL_FILE_PATH` variable in this script
        3. Redeploy your app - the database will be created automatically
        
        **Option 2: For Local Development**
        1. Place your H-1B LCA disclosure Excel file in the project directory
        2. Update these variables in the code:
           ```python
        ...
        **Expected Excel File Format:**
        - Should contain columns like: CASE_STATUS, EMPLOYER_NAME, EMPLOYER_STATE, etc.
        - Typically from U.S. Department of Labor LCA Disclosure Data
        """)
    
    st.info("💡 **Tip**: The database creation is a one-time process. Once created, the dashboard loads instantly!")
...
def main():
    ...
    db_ready, db_message = ensure_database_exists()
    ...
    **🛠️ Technology**: Streamlit + SQLite + Plotly  
